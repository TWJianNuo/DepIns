...
class Trainer:
    ...
    def process_batch(self, inputs, isval = False):
        ...
        losses['totLoss'] = losses['l1loss'] * self.opt.l1lossScale + losses['pholoss'] * self.opt.pholossScale
    ...
    def record_img(self, inputs, outputs):
        ...
        if not self.opt.localGeomMode == 'lidarSupKitti':
            ...
            figcombined = np.concatenate([np.array(figrgb), np.array(figdisp), np.array(fighpred_fromD), np.array(figvpred_fromD)], axis=1)
