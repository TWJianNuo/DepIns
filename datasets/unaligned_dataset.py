...
class UnalignedDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        # self.dir_A = os.path.join(opt.data_path, opt.phase + 'A')  # create a path '/path/to/data/trainA'
        # self.dir_B = os.path.join(opt.data_path, opt.phase + 'B')  # create a path '/path/to/data/trainB'
    ...
    def __getitem__(self, index):
        ...
        """
        A_path = os.path.join(self.opt.data_path, self.A_paths[index % self.A_size])  # make sure index is within then range
        if self.opt.serial_batches:   # make sure index is within then range
            index_B = index % self.B_size
        else:   # randomize the index for domain B to avoid fixed pairs.
            index_B = random.randint(0, self.B_size - 1)
        B_path = os.path.join(self.opt.data_path, self.B_paths[index_B])
