...
from layers import *
from Exp_Cityscape import CityscapeDataset

default_logpath = os.path.join(os.getcwd(), 'tmp')
...
parser.add_argument("--semanticspred_path",     type=str,   default=None)
...
class Trainer:
    def __init__(self, options):
        ...
        self.sfnormOptimizer = SurfaceNormalOptimizer(height=self.orgh, width=self.orgw, batch_size=self.opt.batch_size, angw=self.opt.angw, vlossw=self.opt.vlossw, sclw=self.opt.sclw).cuda()
    ...
    def set_dataset(self):
        ...
        train_dataset = CityscapeDataset(
            self.opt.data_path, train_filenames, self.opt.height, self.opt.width, max_depth=self.opt.max_depth,
            is_train=True, semanticspred_path=self.opt.semanticspred_path
        )

        val_dataset = CityscapeDataset(
            self.opt.data_path, val_filenames, self.opt.height, self.opt.width, max_depth=self.opt.max_depth,
            is_train=False, semanticspred_path=self.opt.semanticspred_path
        )
    ...
    def val(self):
        ...
        with torch.no_grad():
            for batch_idx, inputs in enumerate(self.val_loader):
                ...
                predang = F.interpolate(predang, [self.orgh, self.orgw], mode='bilinear')
    ...
    def record_img(self, inputs, outputs, recoder='train'):
        ...
        vlscolor = F.interpolate(inputs['color'], [self.orgh, self.orgw], mode='bilinear')
