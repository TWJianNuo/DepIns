...
def evaluate(opt):
    ...
    encoder_path = os.path.join('/home/shengjie/Documents/Project_SemanticDepth/tmp/wSyn0_4/models/weights_14', "encoder.pth")
    decoder_path = os.path.join('/home/shengjie/Documents/Project_SemanticDepth/tmp/wSyn0_4/models/weights_14', "depth.pth")
    encoder_dict = torch.load(encoder_path)
    ...
    with torch.no_grad():
        for idx, inputs in enumerate(dataloader):
            ...
            for ind in np.unique(instance_semantic_gt[addmask]):
                ...
                drawX_real_bs = matlab.double(drawX_real_bs.tolist())
                drawY_real_bs = matlab.double(drawY_real_bs.tolist())
                drawZ_real_bs = matlab.double(drawZ_real_bs.tolist())
                ...
                h = eng.scatter3(drawX_real_bs, drawY_real_bs, drawZ_real_bs, 5, 'filled', 'g', nargout=0)
            ...
            #
            # drawX_syn = matlab.double(drawX_syn.tolist())
            # drawY_syn = matlab.double(drawY_syn.tolist())
            # drawZ_syn = matlab.double(drawZ_syn.tolist())
            #
            ...
            #
            # drawX_syn_bs = matlab.double(drawX_syn_bs.tolist())
            # drawY_syn_bs = matlab.double(drawY_syn_bs.tolist())
            # drawZ_syn_bs = matlab.double(drawZ_syn_bs.tolist())
            #
            ...
            #
            # drawX_syn_gt = matlab.double(drawX_syn_gt.tolist())
            # drawY_syn_gt = matlab.double(drawY_syn_gt.tolist())
            # drawZ_syn_gt = matlab.double(drawZ_syn_gt.tolist())


            # eng.eval('figure()', nargout=0)
            # # h = eng.scatter3(drawX_syn, drawY_syn, drawZ_syn, 5, 'filled', 'r', nargout = 0)
            # eng.eval('hold on', nargout=0)
            # h = eng.scatter3(drawX_syn_bs, drawY_syn_bs, drawZ_syn_bs, 5, 'filled', 'g', nargout=0)
            # h = eng.scatter3(drawX_syn_gt, drawY_syn_gt, drawZ_syn_gt, 5, 'filled', 'c', nargout=0)
            # eng.eval('axis equal', nargout = 0)
            # eng.eval('grid off', nargout=0)
            # eng.eval('close all', nargout=0)
            #
