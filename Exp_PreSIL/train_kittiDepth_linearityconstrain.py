...
class Trainer:
    ...
    def constrain_compute_losses(self, inputs, outputs):
        ...
        anghw, angvw = self.imgw.anggradw(angh=inputs['angh'], angv=inputs['angv'])
        wother = self.imgw.rgbgradw(F.interpolate(inputs['color'], [self.opt.crph, self.opt.crpw], mode='bilinear', align_corners=True)) * inputs['semanticsregularmask'] * (outputs[('depth', 0)] < 40).float()
        anghw = anghw * wother
        angvw = angvw * wother
        for scale in range(4):
            l1loss = l1loss + self.gradloss.linearity_consistloss(depthMap=outputs[('depth', scale)], anghw=anghw, angvw=angvw)
            if scale == 0:
    ...
    def record_img(self, inputs, outputs, recoder='train'):
        ...
        fig_anghw = tensor2disp(outputs['anghw'], vmax=1, ind=vind)
        fig_angvw = tensor2disp(outputs['angvw'], vmax=1, ind=vind)

        figoview = np.concatenate([np.array(figrgb), np.array(fig_angh), np.array(fig_angv), np.array(fig_unscaled_disp)], axis=0)
        figanghoview = np.concatenate([np.array(fig_unscaled_disp), np.array(fig_anghw), np.array(fig_gradxdepth), np.array(fig_gradxest)], axis=0)
        figangvoview = np.concatenate([np.array(fig_unscaled_disp), np.array(fig_angvw), np.array(fig_gradydepth), np.array(fig_gradyest)], axis=0)
