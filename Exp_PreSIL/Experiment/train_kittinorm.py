...
parser.add_argument("--ban_shuffle",            action='store_true')
...
class Trainer:
    ...
    def set_dataset(self):
        ...
        self.train_loader = DataLoader(
            train_dataset, self.opt.batch_size, shuffle=not self.opt.ban_shuffle,
    ...
    def norm_compute_losses(self, inputs, outputs):
        ...
        for scale in range(4):
            ...
            prednorm = F.interpolate(prednorm, [self.opt.crph, self.opt.crpw], mode='bilinear', align_corners=True)
            prednorm = F.normalize(prednorm, dim=1)
            integrationloss = self.sfnormOptimizer.intergrationloss_surfacenormal(prednorm, inputs["K"], inputs["depthgt"])
    ...
    def record_img(self, inputs, outputs, recoder='train'):
        ...
        prednorm = (outputs[('disp', 0)] - 0.5) * 2 * np.pi
        prednorm = F.interpolate(prednorm, [self.opt.crph, self.opt.crpw], mode='bilinear', align_corners=True)
        prednorm = F.normalize(prednorm, dim=1)

        predang = self.sfnormOptimizer.normal2ang(surfnorm=prednorm, intrinsic=inputs['K'])
        ...
        fignorm = tensor2rgb((prednorm + 1) / 2, ind=vind)

        figoview = np.concatenate([np.array(figrgb), np.array(fig_anghpred), np.array(fig_angvpred), np.array(fignorm)], axis=0)
