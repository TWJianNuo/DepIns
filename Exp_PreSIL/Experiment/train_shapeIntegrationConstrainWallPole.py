...
class Trainer:
    ...
    def depth_compute_losses(self, inputs, outputs):
        ...
        _, pred_depth = disp_to_depth(outputs[('disp', 0)], min_depth=self.opt.min_depth, max_depth=self.opt.max_depth)
        ...
        semanedgemask = semanedgemask * (pred_depth < 25).int()
        ...
        with torch.no_grad():
            for batch_idx, inputs in enumerate(self.val_loader):
                ...
                for i in range(inputs['color'].shape[0]):
                    ...
                    cur_pred_depth = F.interpolate(cur_pred_depth, [gt_height,gt_width], mode='bilinear', align_corners=False)
