    ...
    def vls(self):
        ...
        with torch.no_grad():
            for batch_idx, inputs in enumerate(self.val_loader):
                ...
                sfnorm_opted = self.sfnormOptimizer.depth2norm(outputs[('depth_opted', 0)], intrinsic=inputs['K'])
                sfnorm_mixed = self.sfnormOptimizer.depth2norm(outputs[('mixed_depth', 0)], intrinsic=inputs['K'])
                ...
                fignorm2 = tensor2rgb((sfnorm_opted + 1) / 2, ind=vind)
                fignorm3 = tensor2rgb((sfnorm_mixed + 1) / 2, ind=vind)

                figoview = np.concatenate([np.array(figd1), np.array(fignorm1), np.array(figconf), np.array(figrgb)], axis=0)
                figanghoview = np.concatenate([np.array(figd2), np.array(fignorm2), np.array(figedge), np.array(figseman)], axis=0)
                figangvoview = np.concatenate([np.array(figd3), np.array(fignorm3), np.array(figmask), np.array(figsemancat)], axis=0)
                ...
                pil.fromarray(figcombined).save(os.path.join('/media/shengjie/c9c81c9f-511c-41c6-bfe0-2fc19666fb32/Visualizations/Project_SemanDepth/singleinput_integrationtest_w1', '{}.png'.format(figname)))
