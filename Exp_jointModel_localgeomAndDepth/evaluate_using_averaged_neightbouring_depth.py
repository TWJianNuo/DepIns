            ...
            for data in dataloader:
                ...
                for i in range(htheta.shape[0]):
                    ...
                    if acckey not in localgeomDict:
                        ...
                        localthetadesp = LocalThetaDesp(height=kittih, width=kittiw, batch_size=1, intrinsic=intrinsicKitti, patchh=opt.patchh, patchw=opt.patchw).cuda()
                    ...
                    if opt.do_theta_optimization:
                        if save_vls_theta:
                            old_theta_h, old_theta_v = localgeomDict[acckey].get_theta(preddepthi)
                        preddepthi = localgeomDict[acckey].optimize_depth_using_theta(depthmap=preddepthi, htheta=hthetai, vtheta = vthetai)
                        if save_vls_theta:
                            new_theta_h, new_theta_v = localgeomDict[acckey].get_theta(preddepthi)
                            fig1 = tensor2disp(old_theta_h - 1, vmax=4, ind=0)
                            fig2 = tensor2disp(new_theta_h - 1, vmax=4, ind=0)
                            fig = np.concatenate([np.array(fig1), np.array(fig2)], axis=0)
                            pil.fromarray(fig).save(os.path.join('/media/shengjie/c9c81c9f-511c-41c6-bfe0-2fc19666fb32/Visualizations/Project_SemanDepth/vls_simple_optimizationOfDepthUsingTheta', str(count) + '.png'))
