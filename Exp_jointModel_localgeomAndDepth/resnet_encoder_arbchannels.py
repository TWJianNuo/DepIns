...
def resnet_multiimage_input(num_layers, pretrained=False, num_input_channels=3):
    ...
    if pretrained:
        ...
        duptime = int((num_input_channels / 3))
        restnum = int(num_input_channels - duptime * 3)

        if restnum == 0:
            conv1w = torch.cat([loaded['conv1.weight']] * duptime, 1)
        else:
            conv1w = torch.cat([loaded['conv1.weight']] * duptime + [loaded['conv1.weight'][:, 0:restnum, :, :]], 1)
        loaded['conv1.weight'] = conv1w
        model.load_state_dict(loaded)
    return model


class ResnetEncoderArbChannels(nn.Module):
    """Pytorch module for a resnet encoder
    """
    def __init__(self, num_layers, pretrained, num_input_channels=3):
        super(ResnetEncoderArbChannels, self).__init__()
