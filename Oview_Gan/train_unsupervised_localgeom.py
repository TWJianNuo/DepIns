...
class Trainer:
    ...
    def constrain_compute_losses(self, inputs, outputs):
        ...
        for i in range(len(self.opt.scales)):
            htheta_pred_detached = outputs[('disp', i)][:, 0:1, :, :] * float(np.pi) * 2
            vtheta_pred_detached = outputs[('disp', i)][:, 1:2, :, :] * float(np.pi) * 2
            htheta_pred_detached = F.interpolate(htheta_pred_detached, [self.opt.height, self.opt.width], mode='bilinear', align_corners=True)
            vtheta_pred_detached = F.interpolate(vtheta_pred_detached, [self.opt.height, self.opt.width], mode='bilinear', align_corners=True)
            ...
            if i == 0:
                outputs['htheta_pred'] = htheta_pred_detached
                outputs['vtheta_pred'] = vtheta_pred_detached
